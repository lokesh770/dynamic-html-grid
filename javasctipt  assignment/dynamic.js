...
const newcontainer = document.getElementById("new-product-grid");
container.style.display="flex";
newcontainer.style.display="flex";
// Loop through the product list and create a card element for each product
productList.forEach((product) => {
  ...
  // Create a new title element and set its text content
  const title = document.createElement("h3");
  title.textContent = product.name;
  
  const brand=document.createElement("h5")
   brand.textContent=product.brand;
  // Create a new price element and set its text content
  const price = document.createElement("h4");
  price.textContent = `RS ${product.price}`;
  price.style.color="#009688";

  // Append the image, title, and price elements to the card element
  card.appendChild(img);
  card.appendChild(title);
  card.appendChild(brand);
  card.appendChild(price);
  // Append the card element to the container div element
